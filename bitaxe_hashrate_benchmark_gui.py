...
# Fixed benchmark constants (not exposed in GUI — safe values)
        ...
        try:
            ...
            while cur_v <= MAX_ALLOWED_VOLTAGE and cur_f <= MAX_ALLOWED_FREQ:
                ...
                if avg_hr is not None:
                    ...
                    if ok:
                        if cur_f + FREQUENCY_INCREMENT <= MAX_ALLOWED_FREQ:
                            cur_f += FREQUENCY_INCREMENT
                        else:
                            self._log("Reached max frequency — benchmark complete.", "green")
                            break
                    else:
                        if cur_v + VOLTAGE_INCREMENT <= MAX_ALLOWED_VOLTAGE:
                            cur_v += VOLTAGE_INCREMENT
                            cur_f  = max(MIN_ALLOWED_FREQ, cur_f - FREQUENCY_INCREMENT)
                            self._log(
